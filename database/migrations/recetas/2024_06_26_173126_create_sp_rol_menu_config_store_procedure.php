...
BEGIN
    ...
    BEGIN
		...
		(rol_id INT, rol_nombre VARCHAR(75), id INT, nombre VARCHAR(75), url VARCHAR(250), icono VARCHAR(75), menu_id INT)

		DECLARE @FinalMenu AS TABLE 
		(rol_id INT, rol_nombre VARCHAR(75), id INT, nombre VARCHAR(75), url VARCHAR(250), icono VARCHAR(75), menu_id INT)

		INSERT INTO @TempMenu (rol_id, rol_nombre, id, nombre, url, icono, menu_id)
        SELECT 
		T1.id AS 'rol_id', T1.nombre AS 'rol_nombre', 
		T2.id AS 'menu_id', T2.nombre AS 'menu_nombre', T2.url AS 'menu_url', T2.icono AS 'menu_icono', T2.menu_id AS 'menu_padre'
		FROM [dbo].[tbl_rol_menu] AS T0
		...
		DECLARE @rol_nombre VARCHAR(75), @id INT, @nombre VARCHAR(75), @url VARCHAR(250), @icono VARCHAR(75), @count INT = (SELECT COUNT(*) FROM @TempMenu);
