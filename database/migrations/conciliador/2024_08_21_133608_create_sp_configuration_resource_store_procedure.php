...
BEGIN
    ...
    AND T0.[COLUMN_NAME] NOT IN ('id', 'plataforma_id', 'plataforma', 'punto_venta', 'alias_id', 'estado', 'plataforma_estado_id', 'created_at', 'created_by', 'procesado', 'informacion')
    ORDER BY T0.[ORDINAL_POSITION];

    SELECT CONCAT(T0.id, CONCAT('. ', UPPER(T2.plataforma), CONCAT(' - ', CONCAT(UPPER(T1.nombre), ' (estado)')))) AS nombre, T0.id AS valor
